"""
allan_analysis_fixed_v2.py
--------------------------
Allan deviation analysis for IMU data.

Run:
    python allan_analysis_fixed_v2.py --input imu_data.csv --out allan_plot.png [--fs 400]
"""
...
def _find_slope_region(tau, adev, target_slope, tol=0.15, min_points=8, tau_min=None, tau_max=None):
    ...
    for i, ok in enumerate(mask):
        if ok and start is None:
            ...
            seg_len = i - start
            if seg_len >= min_points and seg_len > best_len:
                best = (start, i)
                best_len = seg_len
            start = None

    if start is not None:
        seg_len = len(mask) - start
        if seg_len >= min_points and seg_len > best_len:
            best = (start, len(mask))
...
def fit_fixed_slope_auto(tau, adev, slope, tol=0.15, min_points=8, tau_min=None, tau_max=None):
    region = _find_slope_region(
        tau, adev, slope, tol=tol, min_points=min_points, tau_min=tau_min, tau_max=tau_max
    )
    if region is None:
        return np.nan, None
...
def _find_bias_instability_region(tau, adev, min_points=10, tau_min=5.0):
    """
    Look for a genuinely flat Allan-deviation region away from short tau.
    """
    log_tau = np.log10(tau)
    log_adev = np.log10(adev)
    slopes = np.gradient(log_adev, log_tau)

    mask = (np.abs(slopes) <= 0.12) & (tau >= tau_min)
...
def extract_noise_params(tau: np.ndarray, adev: np.ndarray):
    ...
    Extract:
      ...
      - bias instability only if a real flat region exists at larger tau
    """
    off_wn, wn_range = fit_fixed_slope_auto(
        tau, adev, -0.5, tol=0.12, min_points=8, tau_max=5.0
    )
    sigma_wn = np.exp(off_wn) if not np.isnan(off_wn) else np.nan

    off_rw, rw_range = fit_fixed_slope_auto(
    tau, adev, +0.5, tol=0.16, min_points=6, tau_min=1.0
)
    ...
    flat_region = _find_bias_instability_region(tau, adev, min_points=8, tau_min=3.0)
    if flat_region is not None:
        i0, i1 = flat_region
        idx_local = np.argmin(adev[i0:i1])
        idx_bi = i0 + idx_local
        tau_bi = tau[idx_bi]
        bi = adev[idx_bi]
        b_coeff = bi / scf_b
...
def plot_adev_panel(ax_plot, tau, adev_avg, adevs, label, unit, params):
    ...
    if not np.isnan(p["off_wn"]) and p["wn_range"] is not None:
    ...
    if not np.isnan(p["off_rw"]) and p["rw_range"] is not None:
        ...
        tau_k = np.mean(p["rw_range"])
        val_k = fit_line(np.array([tau_k]), p["off_rw"], +0.5)[0]
        ax_plot.scatter([tau_k], [val_k], marker="D", s=70, color="purple", zorder=7)
        ax_plot.annotate(f"K = {p['sigma_rw']:.3e}",
                 xy=(tau_k, val_k),
                 xytext=(tau_k * 1.25, val_k * 0.75),
...
def print_report(input_path, fs, accel_mean, gyro_mean, params_a, params_g):
    sep = "=" * 68
    print(sep)
    ...
    if not np.isnan(params_a["b_coeff"]):
        ...
        print("    Bias instability B:    not detected (no valid flat region at larger τ)")
    print()
